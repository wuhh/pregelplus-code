...
class RWorker{
	...
	public:
			...
			*/
			...
			for(int i=0; i<_num_workers; i++)
			{
				...
				for(RSetIter it=set.begin(); it!=set.end(); it++)
				{
					...
					if(vit!=vmap.end()) message_buffer->add_respond_implicit(vid, i, vit->second->respond());
				}
			}
		}
		...
		void load_vertex(RVertexT* v){//called by load_graph
			add_vertex(v);
		}
		...
		void run(const WorkerParams & params){
			...
			while(true){
				...
				if(hasRR)
				{
					...
					if(_my_rank==MASTER_RANK)
					{
						...
						if(_my_rank==MASTER_RANK) cout<<"-- "<<step_resp_num<<" resps exchanged"<<endl;
					}
				}
				//=================== added for RWorker ===================
			...
			if(_my_rank==MASTER_RANK) cout<<"Total #msgs="<<global_msg_num<<", Total #vadd="<<global_vadd_num<<", Total #reqs="<<global_req_num<<", Total #resps="<<global_resp_num<<endl;
