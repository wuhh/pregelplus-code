...
class RMessageBuffer{
	public:
		...
		void add_request(const KeyT & id){//added for RVertex
			hasMsg();//cannot end yet even every vertex halts
			out_req_sets[hash(id)].insert(id);
		}

		void add_respond_explicit(const KeyT & me, const KeyT & id, const RespondT & respond){//added for RVertex
			hasMsg();//cannot end yet even every vertex halts
			RespMap & rmap=out_resp_maps[hash(id)];
			RespIter it=rmap.find(me);
			if(it==rmap.end()) rmap[me]=respond;
		}

		void add_respond_implicit(const KeyT & me, int wid, const RespondT & respond){//added for RVertex
			hasMsg();//cannot end yet even every vertex halts
		...
		void sync_reqs(){
			for(int i=0; i<_num_workers; i++) in_req_sets[i].clear();//added for RVertex
			all_to_all(out_req_sets, in_req_sets);//added for RVertex
			out_req_sets[_my_rank].swap(in_req_sets[_my_rank]);
			for(int i=0; i<_num_workers; i++) out_req_sets[i].clear();//added for RVertex
		}

		void sync_resps(){
			for(int i=0; i<_num_workers; i++) in_resp_maps[i].clear();//added for RVertex
			all_to_all(out_resp_maps, in_resp_maps);//added for RVertex
			out_resp_maps[_my_rank].swap(in_resp_maps[_my_rank]);
			for(int i=0; i<_num_workers; i++) out_resp_maps[i].clear();//added for RVertex
		}
