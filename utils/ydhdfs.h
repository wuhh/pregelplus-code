...
struct LineReader{
	...
	{
		while(length+num+1>size) doubleLineBuf();
	...
	void fill()
	{
		...
		bufPos=0;
		if(bufSize<HDFS_BUF_SIZE) fileEnd=true;
	}
...
int dirCheck(const char* indir, const char* outdir, bool print, bool force)//returns -1 if fail, 0 if succeed
{
	...
	{
		if(print) fprintf(stderr, "Input path \"%s\" does not exist!\n", indir);
		hdfsDisconnect(fs);
		return -1;
	}
	if(hdfsExists(fs, outdir)==0)
	{
		if(force)
		{
			...
			{
				if(print) fprintf(stderr, "Error deleting %s!\n", outdir);
				exit(-1);
			}
			int created=hdfsCreateDirectory(fs, outdir);
			if(created==-1)
			{
				if(print) fprintf(stderr, "Failed to create folder %s!\n", outdir);
				exit(-1);
			}
		}
		else
		{
			if(print) fprintf(stderr, "Output path \"%s\" already exists!\n", outdir);
			hdfsDisconnect(fs);
			return -1;
		}
	}
...
{//locality is not considered for simplicity
	...
	for(int i=0; i<numSlaves; i++) assigned[i]=0;
	//sort files by size
...
{//locality is not considered for simplicity
	...
	for(int i=0; i<_num_workers; i++) assigned[i]=0;
	//sort files by size
	vector<sizedFName> sizedfile;
	for(int i=0; i<numFiles; i++)
	{
