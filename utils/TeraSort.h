...
class TeraWorker{
	public:
		...
		{
			ResetTimer(4);
			if(_my_rank==MASTER_RANK) cout<<"============= Step 1: Key Sampling ============="<<endl;
			vector<T> samples;
			for(int i=0; i<vertexes.size(); i++)
			{
		...
		{
			if(splits.size()==0) return 0;
			T last=splits.back();
			if(key>last) return _num_workers-1;
			//////
			int pos=0;
			for(int i=0; i<splits.size(); i++)
			{
				if(key<splits[i] || key==splits[i]) return i;
			}
		...
		{
			ResetTimer(4);
			if(_my_rank==MASTER_RANK) cout<<"============= Step 2: Vertex Exchange and Sort ============="<<endl;
		...
		{
			ResetTimer(4);
			if(_my_rank==MASTER_RANK) cout<<"============= Step 3: Prefix Ordering ============="<<endl;
			vector<int> sizes;
			int pre=0;
			if(_my_rank!=MASTER_RANK)
			{
