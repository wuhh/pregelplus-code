...
enum MSTPhase {
    ...
    //DistributedMinEdgePicking_RespondEdgeRoot, //no longer needed
    DistributedMinEdgePicking_Finalize,
    Supervertex_Finding,
    PointerJumping_Request,
    //PointerJumping_Respond,  //no longer needed
    SupervertexFormation_Request,
    //SupervertexFormation_Respond, //no longer needed
...
public:
    ...
    virtual void compute(MessageContainer& messages)
    {
        ...
        case EdgeCleanning_RemoveEdge:
            ...
            if (edges.size() == 0)
                vote_to_halt(); //!!! important in order for agg.ifPointsAtSupervertex to work
            break;
        }
    }
