...
public:
    ...
    virtual void compute(MessageContainer& messages)
    {
        if (step_num() == 1) {
            ...
            int nxt = (power << 1);
            MinMaxValue_req& val = value();
            if (pre % power == 0) { //>>>> glob_request
                intpair minmax_request(id.v1, pre + cur_pow);
                intpair* minmax_resp = get_respond_safe(minmax_request);
                if (minmax_resp != NULL) {
                    if (val.globMin > minmax_resp->v1)
                        val.globMin = minmax_resp->v1;
                    if (val.globMax < minmax_resp->v2)
                        val.globMax = minmax_resp->v2;
                }
            }
            if (val.little < val.big) {
                if (step_num() == 2) {
                    ...
                    if (val.min > localmax_resp.v1)
                        val.min = localmax_resp.v1; //set min(v) = min{min(v), msg.localMin}
                    if (val.max < localmax_resp.v2)
                        val.max = localmax_resp.v2; //set max(v) = max{max(v), msg.localMax}
                }
                if (val.little % power != 0 && step_num() > 2) {
                    //>>>> little_request
                    intpair little_request(id.v1, val.little);
                    intpair little_resp = get_respond(little_request);
                    if (val.min > little_resp.v1)
                        val.min = little_resp.v1; //set min(v) = min{min(v), msg.globMin}
                    if (val.max < little_resp.v2)
                        val.max = little_resp.v2; //set max(v) = max{max(v), msg.globMax}
                    val.little += cur_pow;
                }
                if (val.big % power != 0) {
                    //>>>> big_request
                    ...
                    if (val.min > big_resp.v1)
                        val.min = big_resp.v1; //set min(v) = min{min(v), msg.globMin}
                    if (val.max < big_resp.v2)
                        val.max = big_resp.v2; //set max(v) = max{max(v), msg.globMax}
                    val.big -= cur_pow;
                }
            }
            ////////
