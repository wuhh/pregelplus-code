...
class GVertex
{
	public:
		...
		~GVertex()
		{
			if(tag) delete (vector<int> *)_neighbors;
			else delete (EdgeContainer *)_neighbors;
		}
		...
		inline bool operator < (const GVertexT & rhs) const {return id<rhs.id;}
		inline bool operator==(const GVertexT & rhs) const {return id==rhs.id;}
		inline bool operator!=(const GVertexT & rhs) const {return id!=rhs.id;}
		...
		friend ibinstream & operator<<(ibinstream & m, const GVertexT & v){//just for using by sync_graph()
			m<<v.id;
			m<<v._value;
		...
		friend obinstream & operator>>(obinstream & m, GVertexT & v){//just for using by sync_graph()
			m>>v.id;
			m>>v._value;
			m>>*((EdgeContainer *)v._neighbors);
			m>>v.tag;
