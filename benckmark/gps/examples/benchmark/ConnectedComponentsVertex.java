...
public class ConnectedComponentsVertex extends NullEdgeVertex<IntWritable, IntWritable> {
	...
	public void compute(Iterable<IntWritable> messageValues, int superstepNo) {
		...
		if (superstepNo == 1) {
			...
			for (Edge<NullWritable> e : getOutgoingEdges())
			{
				min = Math.min(min, e.getNeighborId());
			}
	 ...
	 * @author Yi Lu
	 */
	public static class ConnectedComponentsVertexFactory extends NullEdgeVertexFactory<IntWritable, IntWritable> {
