 ...
 */
class graph_coloring : public graphlab::ivertex_program<graph_type,
                                                        set_union_gather>,
...
public:
                       ...
                       edge_type& edge) const
    {
        ...
        color_type other_color = edge.source().id() == vertex.id() ? edge.target().data() : edge.source().data();
                 ...
                 edge_type& edge) const
    {
        ...
        if (edge.source().data() == edge.target().data()) {
            context.signal(edge.source().id() == vertex.id() ? edge.target() : edge.source());
        }
    }
};
